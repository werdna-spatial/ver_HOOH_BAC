...
def main():
    ...
    PD_out=figcom.out
    PD_tag=figcom.tag
    filename=str(PD_tag)+str('map')+str('_AnnualMeanProd.png')
    ...
    #
    #
    fig, axs = plt.subplots(nrows=2,ncols=1,subplot_kw={'projection': ccrs.EckertIV()} , figsize=(18,12) )
    #
    x=ds_tracers['Msum_sa_PHOOH'].values.ravel()
    #print('HOOH :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x)), flush=True, file = sys.stdout)
    print('PHOOH :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x))+' '+str(np.nanpercentile(x,90))+' '+str(np.nanpercentile(x,50))+' '+str(np.nanpercentile(x,10)), flush=True, file = sys.stdout)
    #
    axs[0].set_global()
    axs[0].coastlines()
    ...
    PHOOH_min=0
    PHOOH_max=5.0e-6
    im_a=ds_tracers['Msum_sa_PHOOH'].plot.pcolormesh( x='X', y='Y',
                                               ax=axs[0],
    ...
    #
    #
    x=ds_tracers['Msum_sa_Bdetox'].values.ravel()
    #print('HOOH :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x)), flush=True, file = sys.stdout)
    print('Bdetox :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x))+' '+str(np.nanpercentile(x,90))+' '+str(np.nanpercentile(x,50))+' '+str(np.nanpercentile(x,10)), flush=True, file = sys.stdout)
    #
    axs[1].set_global()
    axs[1].coastlines()
    ...
    Bdetox_min=0
    Bdetox_max=5.0e-6
    im_b=ds_tracers['Msum_sa_Bdetox'].plot.pcolormesh( x='X', y='Y',
                                               ax=axs[1],
                                               transform=ccrs.PlateCarree(),
                                               vmin= Bdetox_min,
                                               vmax= Bdetox_max,
    ...
    cbar_a.set_label('HOOH Production (?)')
    cbar_b = fig.colorbar(im_b, ax=axs[1], shrink=0.7, location='right')
    cbar_b.set_label('Biotic Detox of HOOH(?)')
    #
