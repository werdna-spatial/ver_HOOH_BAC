...
def main():
    ...
    PD_out=figcom.out
    PD_tag=figcom.tag
    filename=str(PD_tag)+str('_map')+str('_MonDep_Hconc.png')
    ...
    for qtr in range(12):
        ...
        for layer in range(3):
            ...
            map_layer=ds_qtr['MAP'].isel(Z=layer)*mole_in_mmol
            ...
            im=map_layer.plot.pcolormesh( x='X', y='Y',
                                                       ...
                                                       zorder=0, 
                                                       cmap=cmap)
            # added layer becaues I am only using one range now and need 1 bar
            if (qtr==0 and layer==2):
                cbar_a = fig.colorbar(im, ax=axs[:,:], location='right')
                cbar_a.set_label('HOOH Concentration (mol HOOH/m^3)')
    #
    #
    #
