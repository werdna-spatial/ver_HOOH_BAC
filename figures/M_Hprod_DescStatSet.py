...
def main():
    ...
    PD_tag=figcom.tag
    dep_layer=2
    filename=str(PD_tag)+str('_m')+str('_Hprod_DescStatSet_DepLayer_'+str(dep_layer)+'.png')
    ...
    #trace_name='P_HOOH'
    trace_name='UTK_D'
    ds_tracers['trace']=ds_tracers[trace_name]*mole_in_mmol*sec_in_day
    #ds_tracers['sa_trace']=ds_tracers['trace']*ds_tracers['DepHeight']
    #ds_tracers['sum_sa_trace']=ds_tracers['sa_trace'].sum(dim=["Z"])
    ds_tracers['mean']=ds_tracers['trace'].mean(dim=["T"])
    ds_tracers['median']=ds_tracers['trace'].median(dim=["T"])
    ds_tracers['max']=ds_tracers['trace'].max(dim=["T"])
    ds_tracers['min']=ds_tracers['trace'].min(dim=["T"])
    ...
    fig, axs = plt.subplots(nrows=4,ncols=1,subplot_kw={'projection': ccrs.EckertIV()} , figsize=(12,20) )
    plt.suptitle('HOOH Production (mol/m^3/day) at Depth layer '+str(dep_layer))
    #
    ...
    maxrange=9.0e-05
    #
    name_list=['max','mean','median','min']
    for index in range(4):
        name_str=name_list[index]
        print(index, name_str)
        map_layer=ds_tracers[name_str]
        x=map_layer.values.ravel()
        #print('HOOH :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x)), flush=True, file = sys.stdout)
        print(name_str+' :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x))+' '+str(np.nanpercentile(x,90))+' '+str(np.nanpercentile(x,50))+' '+str(np.nanpercentile(x,10)), flush=True, file = sys.stdout)
        #
        cmap = plt.get_cmap('hot')
        axs[index].set_global()
        ...
        im=map_layer.plot.pcolormesh( x='X', y='Y',
                                                   ...
                                                   zorder=10, 
                                                   cmap=cmap)
        axs[index].annotate(str(name_str),
    ...
    #
        cbar_a = fig.colorbar(im, ax=axs[index], shrink=0.8, location='right')
        cbar_a.set_label('Yr '+str(name_str)+' HOOH Dep Int Prod  (mol HOOH/m^3/day)')
    #
