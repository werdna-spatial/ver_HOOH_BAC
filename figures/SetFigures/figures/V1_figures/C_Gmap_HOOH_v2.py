...
def main():
    ...
    fig, axs = plt.subplots(nrows=3,ncols=3,subplot_kw={'projection': ccrs.EckertIV()} , figsize=(18,12) )
    ...
    for PD_NC in PD_NC_list:
        ...
        #
        #
        x=ds_tracers['LMsum_sa_hooh'].values.ravel()
        ...
        #
        x=ds_tracers['LMsum_sa_syn'].values.ravel()
        #print('HOOH :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x)), flush=True, file = sys.stdout)
        ...
        #
        x=ds_tracers['LMsum_sa_pro'].values.ravel()
        #print('HOOH :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x)), flush=True, file = sys.stdout)
        ...
        im_a=ds_tracers['LMsum_sa_hooh'].plot.pcolormesh( x='X', y='Y',
                                                   ax=axs[i,0],
        ...
        im_b=ds_tracers['LMsum_sa_syn'].plot.pcolormesh( x='X', y='Y',
                                                   ax=axs[i,1],
                                                   transform=ccrs.PlateCarree(),
                                                   vmin= SYN_min,
                                                   vmax= SYN_max,
        ...
        axs[i,2].coastlines()
        cmap = plt.get_cmap('Greens')
        im_c=ds_tracers['LMsum_sa_pro'].plot.pcolormesh( x='X', y='Y',
                                                   ax=axs[i,2],
                                                   transform=ccrs.PlateCarree(),
                                                   vmin= PRO_min,
                                                   vmax= PRO_max,
    ...
    axs[0,0].annotate('HOOH',
            ...
            color='k',fontsize=20)
    axs[0,1].annotate('Syn',
            xy=(0.5, 1.15), xycoords='axes fraction',
            horizontalalignment='center', verticalalignment='center',
            color='k',fontsize=20)
    axs[0,2].annotate('Pro',
            xy=(0.5, 1.15), xycoords='axes fraction',
            horizontalalignment='center', verticalalignment='center',
            color='k',fontsize=20)
    axs[0,0].annotate('No Impact',
            ...
            rotation=90,
            color='k',fontsize=20)
    axs[1,0].annotate('Kill Pro',
            xy=(-0.15, .5), xycoords='axes fraction',
            horizontalalignment='center', verticalalignment='center',
            rotation=90,
            color='k',fontsize=20)
    axs[2,0].annotate('Save Pro',
            xy=(-0.15, .5), xycoords='axes fraction',
            horizontalalignment='center', verticalalignment='center',
            rotation=90,
            color='k',fontsize=20)
