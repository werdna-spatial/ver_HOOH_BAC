...
def main():
    ...
    PD_out=figcom.out
    PD_tag=figcom.tag
    filename=str(PD_tag)+str('_')+str('HOOH.png')
    ...
    for PD_NC in PD_NC_list:
        ...
        ds_tracers['log_vol_HOOH_mTsZ']=np.log10(ds_tracers['vol_HOOH_mTsZ'])
        x=ds_tracers['log_vol_HOOH_mTsZ'].values.ravel()
        ...
        im_a=ds_tracers['log_vol_HOOH_mTsZ'].plot.pcolormesh( x='X', y='Y',
                                                   ax=axs[i,0],
        ...
        ds_tracers['log_vol_NH4_mTsZ']=np.log10(ds_tracers['vol_NH4_mTsZ'])
        x=ds_tracers['log_vol_NH4_mTsZ'].values.ravel()
        #print('HOOH :: '+str(x.size)+' '+str(np.nanmax(x))+' '+str(np.nanmin(x)), flush=True, file = sys.stdout)
        ...
        im_b=ds_tracers['log_vol_NH4_mTsZ'].plot.pcolormesh( x='X', y='Y',
                                                   ax=axs[i,1],
                                                   transform=ccrs.PlateCarree(),
                                                   vmin= NH4_min,
                                                   vmax= NH4_max,
        ...
        #
        if i==0:
            prev_HOOH=ds_tracers['log_vol_HOOH_mTsZ']
            prev_NH4=ds_tracers['log_vol_NH4_mTsZ']
        i=i+1
        #
        #
    #
    ...
    diff_HOOH=prev_HOOH-ds_tracers['log_vol_HOOH_mTsZ']
    ...
    axs[i,1].coastlines()
    cmap = plt.get_cmap('seismic')
    diff_NH4=prev_NH4-ds_tracers['log_vol_NH4_mTsZ']
    ...
    #
    filename=str('C_Gmap')+str('_mean_HOOH_NH4.png')
